...
onMount(() => {
    // If stored mode is auto, default to light or dark based on preference, or just default to one
    let stored = getStoredTheme();
    if (stored === AUTO_MODE) {
        stored = window.matchMedia("(prefers-color-scheme: dark)").matches ? DARK_MODE : LIGHT_MODE;
        setTheme(stored);
    }
	mode = stored;
});
